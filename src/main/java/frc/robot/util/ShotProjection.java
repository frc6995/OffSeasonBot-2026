 ...
 *
 * <p>Call {@link #solve} ONCE per robot loop and have every subsystem read the public result
 * fields. No objects are created per call: all math is on primitives, lookups are binary searches
 * over primitive arrays (no boxing, unlike InterpolatingDoubleTreeMap), and distance uses
...
public final class ShotProjection {
    ...
    public static final class Lut {
        ...
        public Lut(double[][] data) {
            ...
            for (int i = 0; i < data.length; i++) { x[i] = data[i][0]; y[i] = data[i][1]; }
        }
    ...
    public ShotProjection(double[][] tofData, double[][] hoodData, double[][] rpmData, double[][] passRPMData, double[][] passHoodData) {
     ...
     * @param releaseDelaySec  time from this loop until the ball actually leaves the shooter
     *                         (loop + pose latency + feeder), kept separate from time of flight
     */
    public void solve(double rx, double ry, double rtheta, double vx, double vy,
        ...
        // Start from the real goal, then shift it opposite the robot's velocity by the time of
